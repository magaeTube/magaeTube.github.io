...
* 어떠한 `버전`을 설치할 것인지 
* `직접` 설치할 것인지, `Docker를 사용하여 설치할 것인지 
* 어떠한 `Executor`를 설정할 것인지 
...
&nbsp;이 부분에서는 의견이 갈렸었습니다. `Docker`를 이용하면 그나마 <u>쉽게 구축</u>할 수 있었는데 입사했을 때 듣기로는 회사에서 Docker를 사용하는 분이 없다고 들었습니다. 저는 Docker로 해서 편하게 관리하는 것이 좋겠다고 했었지만 Docker로 하면 유지 보수가 힘들고 빠르게 변화하기가 힘들다는 의견에 몇 번 의견을 나누다가 결국 `직접 구축`하기로 결정하였습니다.

<br><br>

## 어떠한 Executor를 설정할 것인가

&nbsp;Executor의 경우 다른 회사들은 <u>Celery Executor나 Kubernetes Executor</u>를 많이 이용하는 것으로 알고 있었습니다. 회사에서 Docker를 사용하고 있지 않아 Kubernetes를 이용하는 것은 오버 스펙이라고 생각해서 이용하지 않았습니다. Celery를 고민했었는데 구축 당시에 실행되는 프로세스가 적었기 때문에 Local Executor로도 충분하겠다고 생각하여 `Local Executor`를 선택하였습니다.
...
&nbsp;가상환경을 실행하고 당시의 Airflow 최신 버전인 1.10.14를 설치하였습니다. Airflow를 설치하고 초기화를 하면 `airflow.cfg` 파일이 생성되는데 해당 파일 안에  Metadata Database와 Executor 등 몇 가지 설정을 해줍니다.
...
&nbsp;첫 번째로 Airflow를 실행할 때 참고하는 설정 파일 `airflow.cfg`를 변경해야 합니다.
...
&nbsp;기본적으로 각각의 값은 `utc`로 되어있는데 이를 변경합니다. 변경할 때에는 kst가 아니라 <u>IANA timezone 데이터</u>인 `Asia/Seoul`로 변경을 해야 합니다. Asia/Seoul이 아니라 다른 곳에 계시다면 <a href="https://secure.jadeworld.com/JADETech/JADE2020/OnlineDocumentation/content/resources/encyclosys2/jadetimezone_class/ianawindowstimezonemapping.htm">링크</a>를 통해 맞는 정보를 찾아서 적용하시면 됩니다.

<br><br>

### DAG에서의 Timezone 변경

&nbsp;Airflow 설정을 통해 Timezone을 변경하였다면 DAG를 작성할 때에도 Timezone을 변경해야 합니다. DAG를 작성할 때 <u>start_date를 참조</u>하는데 이때 설정한 Timezone에 따라 영향이 가기 때문입니다. 잘못하면 내가 원하지 않은 시간에 실행이 될 수 있습니다. 변경하는 방법에는 파이썬 패키지 중 하나인 `pendulum`을 이용합니다. pendulum은 Airflow를 설치하면 자동으로 같이 설치됩니다.
...
# 알림 받기
&nbsp;기본적으로 Airflow를 구축하고 기존에 크론잡을 통해 실행되던 프로세스를 <u>DAG로 작성</u>하여 옮겼습니다. DAG들이 실행되면서 업무시간에는 Webserver를 띄워놓은 채로 새로고침하며 모니터링했습니다. (초반에는 무식하게 꾸준히...) 이렇게 하염없이 바라보기에는 다른 업무들도 해야 했기에 알림을 받는 게 필요하다고 생각했습니다. 기존에 레포트 결과 송신용 `이메일 계정`이 따로 있었기에 이를 이용하기로 했습니다. 

<br><br>

## 설정
&nbsp;이메일의 경우 `airflow.cfg`에서 설정을 해야 합니다. 
...
## 개발
&nbsp;DAG를 생성할 때 이메일의 경우 `default_args`에 내용을 추가하면 됩니다.
...
## catchup
&nbsp;크론잡에서 실행 중이던 프로세스를 신규 DAG로 옮겨왔습니다. 이전부터 실행되던 프로세스였기 때문에 <u>start_date를 과거로 설정</u>하였습니다. 그러고 Airflow에 등록하였더니 해당 DAG에 큐가 엄청 쌓이더니 실행이 되었습니다. 내용을 확인해 보니 <u>start_date부터 현재까지 지정한 interval로 큐가 쌓인 것</u>이었습니다. 필요에 따라 이런 경우가 필요할 수 있지만 이렇게 과거에 실행되지 않았던 부분을 자동으로 실행되지 않기 위해서는 `catchup`이라는 설정을 변경해야 합니다. 
...
&nbsp;`depends_on_past`의 경우 default_args을 설정할 때 지정합니다. 해당 속성을 지정하여 위와 같은 문제를 해결했습니다.
